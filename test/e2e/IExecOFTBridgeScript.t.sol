...
import {Deploy as IExecOFTBridgeDeploy} from "../../script/IExecOFTBridge.s.sol";
...
contract RLCOFTScriptTest is Test {
    ...
    address tokenChainX; // This will be set to a mock token address for testing
    address LAYERZERO_ENDPOINT = 0x6EDCE65403992e310A62460808c4b910D972f10f; // LayerZero Arbitrum Sepolia endpoint

    IExecOFTBridgeDeploy public deployer;

    function setUp() public {
        vm.createSelectFork(vm.envString("ARBITRUM_SEPOLIA_RPC_URL")); // use public node
        deployer = new IExecOFTBridgeDeploy();
        tokenChainX = address(new RLCMock("TokenChainX", "RLC"));
    ...
    function testFork_CheckDeployment() public {
        ...
        IExecOFTBridge rlcoft = IExecOFTBridge(deployer.deploy(tokenChainX, LAYERZERO_ENDPOINT, owner, pauser, salt));

        assertEq(rlcoft.owner(), owner);
        assertEq(rlcoft.token(), address(tokenChainX));
    }
    ...
    //     address addr1 = deployer.deploy(tokenChainX, LAYERZERO_ENDPOINT, owner, pauser, salt1);
    //     address addr2 = deployer.deploy(tokenChainX, LAYERZERO_ENDPOINT, owner, pauser, salt2);
    ...
    //     // First deployment
    //     address addr = deployer.deploy(tokenChainX, LAYERZERO_ENDPOINT, owner, pauser, salt);
    //     assertTrue(addr != address(0), "First deployment should succeed");

    //     // Attempt redeployment with the same salt
    //     try deployer.deploy(tokenChainX, LAYERZERO_ENDPOINT, owner, pauser, salt) returns (address) {
