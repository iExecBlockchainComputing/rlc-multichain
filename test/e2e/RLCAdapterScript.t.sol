...
contract RLCAdapterScriptTest is Test {
    ...
    function testFork_CheckDeployment() public {
        ...
        RLCAdapter rlcAdapter = RLCAdapter(deployer.deploy(lzEndpoint, owner, pauser, CREATEX_FACTORY, salt, RLC_TOKEN));
    ...
    function testForkFuzz_DifferentSaltsProduceDifferentAddresses(bytes32 salt1, bytes32 salt2) public {
        ...
        address addr1 = deployer.deploy(lzEndpoint, owner, pauser, CREATEX_FACTORY, salt1, RLC_TOKEN);
        address addr2 = deployer.deploy(lzEndpoint, owner, pauser, CREATEX_FACTORY, salt2, RLC_TOKEN);
    ...
    function testForkFuzz_RevertIfSecondDeploymentWithSameSalt(bytes32 salt) public {
        // First deployment
        address addr = deployer.deploy(lzEndpoint, owner, pauser, CREATEX_FACTORY, salt, RLC_TOKEN);
        assertTrue(addr != address(0), "First deployment should succeed");

        // Attempt redeployment with the same salt
        try deployer.deploy(lzEndpoint, owner, pauser, CREATEX_FACTORY, salt, RLC_TOKEN) returns (address) {
