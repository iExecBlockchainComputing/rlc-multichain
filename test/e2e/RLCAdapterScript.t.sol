...
contract RLCAdapterScriptTest is Test {
    ...
    function setUp() public {
        vm.createSelectFork("https://sepolia.gateway.tenderly.co/56OqCbuPte15zM70pAt1Ws"); // use public node
    ...
    function testForkFuzz_RevertIfSecondDeploymentWithSameSalt(bytes32 salt) public {
        // First deployment
        address addr = deployer.deploy(lzEndpoint, owner, CREATEX_FACTORY, salt, RLC_TOKEN);
        assertTrue(addr != address(0), "First deployment should succeed");

        // Attempt redeployment with the same salt
        try deployer.deploy(lzEndpoint, owner, CREATEX_FACTORY, salt, RLC_TOKEN) returns (address) {
