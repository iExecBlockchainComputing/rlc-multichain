...
contract TestableBeginTransferAdminRole is BeginTransferAdminRole {
    ...
    function publicBeginTransferAsAdmin(address contractAddress, address newAdmin, string memory contractName, address admin) public {
        vm.startPrank(admin);
        beginTransfer(contractAddress, newAdmin, contractName);
        vm.stopPrank();
    }
...
contract TestableAcceptAdminRole is AcceptAdminRole {
    ...
    function publicAcceptContractAdminAsUser(address contractAddress, string memory contractName, address user) public {
        vm.startPrank(user);
        acceptContractAdmin(contractAddress, contractName);
        vm.stopPrank();
    }
...
contract TransferAdminRoleScriptTest is TestHelperOz5 {
    ...
    function test_RevertWhen_NewAdminIsZeroAddress() public {
        ...
        beginTransferScript.publicBeginTransfer(
    ...
    function test_BeginTransferAdminRole_LiquidityUnifier() public {
        assertEq(
            IAccessControlDefaultAdminRules(address(rlcLiquidityUnifier)).defaultAdmin(),
            admin
        );
        beginTransferScript.publicBeginTransferAsAdmin(
            address(rlcLiquidityUnifier),
    ...
    function test_BeginTransferAdminRole_CrosschainToken() public {
        assertEq(
            IAccessControlDefaultAdminRules(address(rlcCrosschainToken)).defaultAdmin(),
            admin
        );
        beginTransferScript.publicBeginTransferAsAdmin(
