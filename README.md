...
This project includes a comprehensive Safe multisig integration for managing bridge operations through secure multi-signature transactions.
...
2. Configure Safe environment:
   ```bash
   cp .env.safe.template .env.safe
   # Edit .env.safe with your Safe configuration
   ```

3. Required configuration:
   - `SAFE_ADDRESS`: Your Safe multisig address
   - `RPC_URL`: Network RPC endpoint
   - `OWNER_1_ADDRESS` & `OWNER_1_PRIVATE_KEY`: First owner credentials
   - `OWNER_2_ADDRESS` & `OWNER_2_PRIVATE_KEY`: Second owner credentials

#### Available Commands

**Propose transactions:**
```bash
# Generic transaction proposal
...
Use the Safe web interface at https://app.safe.global/ to confirm and execute proposed transactions.

For detailed usage and examples, see [safe/README.md](safe/README.md).
