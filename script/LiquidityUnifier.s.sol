...
contract Deploy is Script {
    ...
    function run() external returns (address) {
        ...
        vm.startBroadcast();
        address liquidityUnifierProxy = deploy(admin, upgrader, createxFactory, createxSalt);
        vm.stopBroadcast();

        //TODO: use config file to store addresses.
        EnvUtils.updateEnvVariable("RLC_CROSSCHAIN_ADDRESS", liquidityUnifierProxy);
     ...
     * @return address of the deployed LiquidityUnifier proxy contract.
     */
    function deploy(address admin, address upgrader, address createxFactory, bytes32 createxSalt)
        public
        returns (address)
    {
        bytes memory initData = abi.encodeWithSelector(LiquidityUnifier.initialize.selector, admin, upgrader);
        return
            UUPSProxyDeployer.deployUUPSProxyWithCreateX("LiquidityUnifier", "", initData, createxFactory, createxSalt);
    }
}
